"""BMv2 simple_switch_CLI helpers for register access."""
...
EXPORT_MODE_ACTIONS: dict[str, str] = {
...
def set_telemetry_export_mode(thrift_port: int, mode: str) -> None:
    action = EXPORT_MODE_ACTIONS.get(mode)
    if action is None:
        raise ValueError(f"unknown export mode {mode!r}; expected one of {sorted(EXPORT_MODE_ACTIONS)}")
    commands = [
        "table_clear MyIngress.configure_export_mode",
        f"table_add MyIngress.configure_export_mode {action} 1 =>",
    ]
    run_cli(thrift_port, commands)
    if mode in ("udp", "both"):
