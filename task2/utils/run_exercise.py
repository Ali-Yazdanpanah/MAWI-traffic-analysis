...
import p4runtime_lib.pipeline_config as p4runtime_pipeline
    ...
    def __init__(self, hosts, switches, links, log_dir, bmv2_exe, pcap_dir, **opts):
        ...
        for sw, params in switches.items():
            if "program" in params:
                # gRPC workflow: load P4 only via P4Runtime (not CLI JSON arg).
                use_p4runtime = "grpc" in bmv2_exe and "runtime_json" in params
                json_path = None if use_p4runtime else params["program"]
                switchClass = configureP4Switch(
                        sw_path=bmv2_exe,
                        json_path=json_path,
                        # log_console=True,  # trace logging slows BMv2; enable only for P4 debug
                        pcap_dump=pcap_dir)
            else:
                # add default switch
                switchClass = None
...
class ExerciseRunner:
    ...
    def create_network(self):
        ...
        defaultSwitchClass = configureP4Switch(
                                ...
                                json_path=None if 'grpc' in self.bmv2_exe else self.switch_json,
                                # log_console=True,  # trace logging slows BMv2; enable only for P4 debug
    ...
    def program_switch_p4runtime(self, sw_name, sw_dict):
        """Push P4 pipeline to simple_switch_grpc via P4Runtime."""
        ...
        try:
            p4info_path, bmv2_json_path = p4runtime_pipeline.load_runtime_paths(
                runtime_json,
                workdir=os.getcwd(),
            )
            self.logger('  p4info: %s' % p4info_path)
            self.logger('  bmv2_json: %s (%d bytes)' % (bmv2_json_path, bmv2_json_path.stat().st_size))
            p4runtime_pipeline.push_forwarding_pipeline(
            ...
            if os.path.isfile(log_path):
                self.logger('Last lines of %s:' % log_path)
    ...
    def program_switches(self):
        ...
        for sw_name, sw_dict in self.switches.items():
            ...
            elif 'grpc' in self.bmv2_exe and 'program' in sw_dict:
                auto = dict(sw_dict)
                auto['runtime_json'] = 'topo/task2-p4runtime.json'
                self.program_switch_p4runtime(sw_name, auto)
            elif 'cli_input' not in sw_dict:
